...
# Creating pixel grid
start = cells / (2 * resolution)
stop = cells - start

pixel_axis = np.linspace(start, stop, resolution)
pix_xx, pix_yy = np.meshgrid(pixel_axis, pixel_axis)
...
angles = 2 * math.pi * np.random.rand(nodes, nodes)
...
pixels_per_cell = resolution / cells
delta = pixels_per_cell
grad = { # each matrix extends the node-size angle matrix to pixel-size map from pixel->corresponding angle
    "top-left": angles[:-1, :-1].repeat(delta, 0).repeat(delta, 1), # delta+1 because [a].repeat(2)=[a,a]
